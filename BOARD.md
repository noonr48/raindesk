...
**Design:** the Mill (overnight candidate generation on SSbenbi3 + ComfyUI/MiniMax-H3) + the Push (one verb-only daily Vault message) + ingest (owner reply → agent turn → board update → commit via roompc bridge).
