...
/**
 * Raindesk backend â€” node:http only, zero deps. Binds all interfaces
 * (0.0.0.0:17600 by default, override with RAINDESK_HOST) so the owner's
 * phone/laptop reach it over the tailnet â€” same exposure model as the
 ...
 * (bounds resource use on the trusted network â€” not a hard security limit).
 */
...
// Two â‰¤20MB PNGs travel base64 in one JSON body (~27MB each decoded-then-encoded).
...
const CHAT_CONCURRENCY = 3; // each chat spawns a pi process â€” cap concurrent spawns
let chatInFlight = 0;
