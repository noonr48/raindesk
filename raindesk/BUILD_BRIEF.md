...
- **Backend**: Node.js ≥18, zero-build, `server.js` + `lib/` modules. Serves static `public/` and the JSON API on **127.0.0.1:17600** (loopback; tailscale exposes it). [Built deviation, commit 716b5cd: binds `0.0.0.0:17600` with `RAINDESK_HOST` override for direct tailnet phone delivery — server.js:40; the brief's loopback intent is preserved in that the API is not internet-exposed.] No framework beyond node:http if possible (deps allowed only if genuinely needed; prefer none).
