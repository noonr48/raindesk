...
- **Backend**: Node.js ≥18, zero-build, `server.js` + `lib/` modules. Serves static `public/` and the JSON API on **127.0.0.1:17600** (loopback; private-mesh exposes it). No framework beyond node:http if possible (deps allowed only if genuinely needed; prefer none).
