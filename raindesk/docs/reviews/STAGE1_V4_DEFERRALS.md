...
- **Mutation ledger bodyHash** (added post-d074732): same (windowId, generation, mutationId) with a DIFFERENT patch now refuses `409 MUTATION_ID_REUSED` instead of acking an unapplied patch. canonicalJson sorts keys recursively, so key-reordered but semantically equal bodies hash identically and replay; the residual limitation is bodies diverging only in fields this route ignores — they refuse 409 rather than replay.
