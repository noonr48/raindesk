...
function createPartner({ agentImpl, directionImpl = direction } = {}) {
  ...
  async function turn({ message = '', mode = null, context = {} } = {}) {
    ...
    const prompt = buildPrompt({ message, summary, extraContext: context, kickstart });
    let rawReply = '';
    ...
    return { ...envelope, intentId: intent ? intent.id : null };
  }
...
module.exports = {
  ...
  looksStuck, kickstartSeed, buildPrompt, createPartner,
};
