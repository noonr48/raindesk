 ...
 *
 * SequenceCandidateManifest stays immutable. This ledger stores owner review
 * events separately and derives current presentation/acceptance state from the
 * event log. Candidate/project/sequence/snapshot identity is always resolved
 * from the locally imported candidate record.
 *
 * `combine` intentionally fails closed in this slice. The previous scaffold
 * accepted mutable Direction Graph annotation ids as ReviewDecision refs; that
 * conflated directing intent with candidate-bound review evidence. Combine is
 * enabled only after immutable ReviewAnnotation records exist.
 */
...
function emptyStore() {
  return { schemaVersion: STORE_SCHEMA_VERSION, decisions: [], idempotency: {}, createdAt: now(), updatedAt: now() };
}
...
function canonicalValue(value) {
  ...
  if (value && typeof value === 'object') {
    const out = {};
    for (const key of Object.keys(value).sort()) if (value[key] !== undefined) out[key] = canonicalValue(value[key]);
    return out;
  }
  return value;
}
...
function read() {
  ...
  let store;
  try { store = JSON.parse(raw); } catch (_error) { throw new HttpError(500, 'animatic review ledger is corrupt'); }
...
function write(store) {
  if (store.decisions.length > MAX_DECISIONS) throw new HttpError(507, 'animatic review ledger reached its v1 decision limit');
  store.updatedAt = now();
  atomicWrite(store);
  return store;
}
...
function candidateAuthority(candidateId) {
  ...
  if (!manifest || manifest.schema_version !== CONTRACT_SCHEMA_VERSION) throw new HttpError(409, 'candidate is not an imported SequenceCandidateManifest@0.2.0');
...
function assertReviewEvidence(decision, annotationRefs) {
  if (annotationRefs != null && (!Array.isArray(annotationRefs) || annotationRefs.length)) {
    throw new HttpError(409, 'candidate-bound ReviewAnnotation evidence is not implemented yet');
  }
  if (decision === 'combine') {
    throw new HttpError(409, 'Combine needs candidate-bound review notes and is not available yet');
  }
  return [];
}

function fingerprintInput({ candidateId, decision, note }) {
  return sha256(canonicalJson({ candidateId, decision, note: note || '' }));
}
...
function latestForCandidate(decisions, candidateId) {
  for (let i = decisions.length - 1; i >= 0; i -= 1) if (decisions[i].candidate_id === candidateId) return decisions[i];
  return null;
}
...
function append({ candidateId, decision, note = null, annotationRefs = null, idempotencyKey } = {}) {
  ...
  assertReviewEvidence(choice, annotationRefs);
  const authority = candidateAuthority(candidateId);
  const store = read();
  const fingerprint = fingerprintInput({ candidateId: authority.manifest.candidate_id, decision: choice, note: cleanNote });
  ...
  const event = {
    ...
    annotation_refs: [],
    supersedes_decision_id: chooseSupersedes(store, authority, choice),
    note: cleanNote,
  ...
  if (!ID_RE.test(event.decision_id) || !ID_RE.test(event.candidate_id) ||
      ...
      (event.sequence_id !== null && !ID_RE.test(event.sequence_id)) || !SHA_RE.test(event.source_snapshot_digest)) {
...
module.exports = {
  ...
  DECISIONS, ID_RE, IDEMPOTENCY_RE, SHA_RE, MAX_DECISIONS, MAX_NOTE,
  canonicalValue, canonicalJson, read, write, candidateAuthority,
  assertReviewEvidence, fingerprintInput, decisionId, activeKeep,
  latestForCandidate, sequenceDecisions, chooseSupersedes,
